...
const Footer = () => {
  ...
  useEffect(() => {
    const tickerSections = (sections || []).filter(
      (section) => section.section_key === "news_ticker",
    );
    const footerSections = (sections || []).filter(
      (section) => section.section_key === "footer",
    );

    if (footerSections.length > 0 || tickerSections.length > 0) {
      const about = footerSections.find(
        (item) => item.type === "about" && item.is_active,
      );
      const contact = footerSections.find(
        (item) => item.type === "contact" && item.is_active,
      );
      const rights = footerSections.find(
        (item) => item.type === "rights" && item.is_active,
      );
  ...
  const handleNewsClick = (newsItem) => {
    ...
    const mediaSectionItem = (sections || []).find(
      (s) =>
        (s.section_key === "media" ||
          s.type === "media" ||
          s.section_key === "news_media") &&
    ...
    if (mediaSectionItem) {
      ...
      console.warn(
        "No matching media item found for title:",
        newsItem.title_en,
      );
    }
  };
      ...
      <div className={styles.container}>
        <div className={styles.footerGrid}>
          ...
          <div className={styles.footerColumn}>
            ...
            <p className={styles.description}>
              {footerData.about
                ? isAr
                  ? footerData.about.title_ar
                  : footerData.about.title_en
                : getTranslation(
                    "footer.aboutText",
                    "Leading the way in infrastructure, construction, and logistics since 1980.",
                  )}
            </p>
          </div>
          ...
          <div className={styles.footerColumn}>
            ...
            <ul>
              <li>
                <Link href="/">{getTranslation("nav.home", "Home")}</Link>
              </li>
              <li>
                <Link href="/about">
                  {getTranslation("nav.companySub.about", "About us")}
                </Link>
              </li>
              <li>
                <Link href="/projects">
                  {getTranslation("nav.projects", "Our Projects")}
                </Link>
              </li>
              <li>
                <Link href="/services">
                  {getTranslation("nav.services", "Our Services")}
                </Link>
              </li>
              <li>
                <Link href="/contact">
                  {getTranslation("nav.contact", "Contact Us")}
                </Link>
              </li>
            </ul>
          </div>
              ...
              {footerData.news.length > 0 ? (
                ...
                <>
                  <li>
                    {getTranslation(
                      "footer.news1",
                      "Signing an agreement with Roshn Group",
                    )}
                  </li>
                  <li>
                    {getTranslation(
                      "footer.news2",
                      "Saudi Arabia wins to host 2034 World Cup",
                    )}
                  </li>
                </>
              )}
          ...
          <div className={styles.footerColumn}>
            ...
            <div className={styles.contactWrapper}>
                ...
                <div className={styles.contactInfoText}>
                  ...
                  <span>
                    {footerData.contact && footerData.contact.details?.address
                      ? isAr
                        ? footerData.contact.details.address.ar
                        : footerData.contact.details.address.en
                      : getTranslation(
                          "footer.addressText",
                          "Saudi Arabia - Riyadh",
                        )}
              ...
              <div className={styles.contactItem}>
                ...
                <div className={styles.contactInfoText}>
                  <label>
                    {getTranslation("footer.workingHours", "Working Hours")}
                  </label>
                  <span className={styles.workHours}>
                    {footerData.contact &&
                    footerData.contact.details?.hours?.sat
                      ? isAr
                        ? footerData.contact.details.hours.sat.ar
                        : footerData.contact.details.hours.sat.en
                      : getTranslation("footer.sat", "Saturday: 09:00 - 14:00")}
                  </span>
                  <span className={styles.workHours}>
                    {footerData.contact &&
                    footerData.contact.details?.hours?.week
                      ? isAr
                        ? footerData.contact.details.hours.week.ar
                        : footerData.contact.details.hours.week.en
                      : getTranslation(
                          "footer.sunThu",
                          "Sun - Thu: 08:00 - 17:00",
                        )}
        ...
        <div className={styles.footerBottom}>
          <p>
            ...
            {footerData.rights
              ? isAr
                ? footerData.rights.title_ar
                : footerData.rights.title_en
              : getTranslation(
                  "footer.rights",
                  "Alajmi Company. All Rights Reserved.",
                )}
          </p>
        </div>
      </div>
