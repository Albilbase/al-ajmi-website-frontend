...
export default function ServicesManager() {
    ...
    const fetchAllData = () => {
      ...
      try {
        if (sections && sections.length > 0) {
          // Fetch service items
          const servicesSections = sections.filter(s => s.section_key === 'home' && s.type === 'service');
          ...
          // Fetch banner
          const bannerSection = sections.find(s => s.section_key === 'home' && s.type === 'service_banner');
          if (bannerSection) {
            setBanner({
              id: bannerSection.id,
          ...
          const headerSection = sections.find(s => s.section_key === 'home' && s.type === 'service_header');
          if (headerSection) {
            setSectionHeader({
              id: headerSection.id,
  ...
  const removeImage = async (type, id = null) => {
    if (type === 'service') {
      const currentService = services[activeItem];
      // Local preview only
      if (editorFile || (currentService?.image && currentService.image.startsWith('blob:'))) {
        setEditorFile(null);
        setEditorPreview(null);
      ...
      if (currentService && currentService.image && currentService.id) {
        if (window.confirm("حذف صورة الخدمة نهائياً من السيرفر؟")) {
          try {
