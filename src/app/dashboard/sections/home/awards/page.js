...
export default function AwardsManager() {
    ...
    const fetchAllData = () => {
      ...
      try {
        if (sections && sections.length > 0) {
          // Fetch award items
          const awardsSections = sections.filter(
            (s) => s.section_key === "home" && s.type === "award",
          );
            ...
            const mappedAwards = awardsSections.map((s) => ({
              ...
              src:
                s.images && s.images.length > 0
                  ? `http://192.168.15.95:5000${s.images[s.images.length - 1]}`
                  : null,
              rawImage:
                s.images && s.images.length > 0
          ...
          const headerSection = sections.find(
            (s) => s.section_key === "home" && s.type === "award_header",
          );
          if (headerSection) {
            setSectionHeader({
              id: headerSection.id,
  ...
  const handleAddAward = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "حدث خطأ أثناء إضافة الجائزة",
      );
    } finally {
  ...
  const removeImage = async () => {
    ...
    if (
      editorFile ||
      (currentAward?.src && currentAward.src.startsWith("blob:"))
    ) {
      setEditorFile(null);
      setEditorPreview(null);
    ...
    if (currentAward.id && currentAward.src) {
      if (window.confirm("حذف صورة الجائزة نهائياً من السيرفر؟")) {
        try {
          const rawPath =
            currentAward.rawImage ||
            currentAward.src.replace("http://192.168.15.95:5000", "");
  ...
  const handleSaveSectionHeader = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "حدث خطأ أثناء حفظ عنوان القسم",
      );
    } finally {
      setIsSubmitting(false);
    }
      ...
      <div className={localStyles.header}>
        <div>
          ...
          <p className={dashboardStyles.sectionSubtitle}>
            Manage the accolades and certifications showcased on your homepage.
          </p>
        </div>
          ...
          >
            ...
            <div className={localStyles.itemsList}>
              {awards.length === 0 ? (
                ...
                >
                  No awards found. Add one to get started.
                </p>
              ) : (
            ...
            <div className={dashboardStyles.contentCard}>
              <div className={localStyles.editorHeader}>
                <h3 style={{ fontSize: "1.25rem", fontWeight: "800" }}>
                  Editing: {awards[activeItem]?.title_en}
                </h3>
                <button
              ...
              <div className={localStyles.formGrid}>
                <div className={localStyles.inputGroup}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateActiveAward("title_en", e.target.value)
                    }
                ...
                <div dir="rtl" className={localStyles.inputGroup}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateActiveAward("title_ar", e.target.value)
                    }
                    className={localStyles.inputField}
                    style={{ fontWeight: "700" }}
                  />
              ...
              <div className={localStyles.formGrid}>
                <div className={localStyles.inputGroup}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateActiveAward("category_en", e.target.value)
                    }
                    className={localStyles.inputField}
                  />
                </div>
                <div dir="rtl" className={localStyles.inputGroup}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateActiveAward("category_ar", e.target.value)
                    }
                    className={localStyles.inputField}
                  />
                </div>
              ...
              <div className={localStyles.mediaSection}>
                ...
                <div className={localStyles.mediaGrid}>
                  <div className={localStyles.mediaPreview}>
                    ...
                    <div
                      className={localStyles.mediaOverlay}
                      style={{ opacity: 1 }}
                    >
                      <div style={{ display: "flex", gap: "0.5rem" }}>
                        <label
                          className={localStyles.changeMediaBtn}
                  ...
                  <div className={localStyles.mediaInfoBox}>
                    <p className={localStyles.mediaInfoText}>
                      ...
                      {awards[activeItem].src || "No image uploaded"} <br />
                      <br />
