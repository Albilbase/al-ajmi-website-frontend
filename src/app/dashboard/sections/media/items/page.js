...
import RichTextEditor, {
  stripHtml,
} from "../../../_components/RichTextEditor/RichTextEditor";
...
export default function MediaManager() {
  ...
  const handleSave = async () => {
    ...
    const hasGallery =
      (currentItem.sliderImages?.length || 0) > 0 ||
    ...
    if (Object.keys(errors).length > 0) {
      ...
      toast.error(
        "Please fill in all required fields and add at least one gallery item",
      );
      return;
    }
    ...
    const details = {
      ...
      videoIframes:
        currentItem.videoIframes?.filter((link) => link.trim() !== "") || [],
    };
  ...
  const updateField = (lang, field, value) => {
    ...
    if (formErrors[`item_${field}_${lang}`])
      setFormErrors({ ...formErrors, [`item_${field}_${lang}`]: false });
  };
  ...
  const handleSliderImagesUpload = (e) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (validFiles.length > 0) {
        ...
        if (formErrors.item_slider)
          setFormErrors({ ...formErrors, item_slider: false });
      }
    }
  };
  ...
  const removeSliderImage = async (index, isExisting) => {
    ...
    if (isExisting) {
      ...
      const result = await confirmDelete(
        ...
        `Are you sure you want to delete this ${itemTypeLabelLower}?`,
      );
      if (result.isConfirmed) {
        try {
          await deleteImageAPI(currentItem.id, imageName);
          setCurrentItem((prev) => {
            const newSliderImages = prev.sliderImages.filter(
              (_, i) => i !== index,
            );
            const newRawSliderImages = prev.rawSliderImages.filter(
              (_, i) => i !== index,
            );
            return {
              ...prev,
              sliderImages: newSliderImages,
              rawSliderImages: newRawSliderImages,
            };
          });
          ...
          setData((prev) => {
            ...
            const newItems = [...prev.items];
            newItems[idx].sliderImages = newItems[idx].sliderImages.filter(
              (_, i) => i !== index,
            );
            newItems[idx].rawSliderImages = newItems[
              idx
            ].rawSliderImages.filter((_, i) => i !== index);
            return { ...prev, items: newItems };
          });
          ...
          toast.error(
            `An error occurred while deleting the ${itemTypeLabelLower}`,
          );
        }
      }
    } else {
  ...
  const handleSaveBanner = async () => {
    ...
    try {
      ...
      setData((prev) => ({
        ...
        rawBannerImage: response.data.images?.[0],
      }));
      setBannerFile(null);
      ...
      <div className={localStyles.header}>
        <div>
          ...
          <p className={dashboardStyles.sectionSubtitle}>
            Manage news, events, and your media gallery.
          </p>
        </div>
              ...
              >
                <div className={localStyles.imageWrapper}>
                  <img
                    src={item.image}
                    alt={item.en.title}
                    className={localStyles.cardImage}
                  />
                ...
                <div className={localStyles.cardContent}>
                  ...
                  <div className={localStyles.cardDesc}>
                    {stripHtml(item.en.description)}
                  </div>
                </div>
                <div className={localStyles.cardActions}>
                  <button
                    className={`${localStyles.actionBtn} ${localStyles.editBtn}`}
          ...
          <div className={localStyles.modalContent}>
            ...
            >
              ...
              >
                ...
                <input
                  className={localStyles.input}
                  value={currentItem.tag_en}
                  onChange={(e) =>
                    setCurrentItem({ ...currentItem, tag_en: e.target.value })
                  }
              ...
              >
                ...
                <input
                  className={localStyles.input}
                  value={currentItem.tag_ar}
                  onChange={(e) =>
                    setCurrentItem({ ...currentItem, tag_ar: e.target.value })
                  }
                  placeholder="مثلاً: أخبار"
                />
              </div>
            </div>
            ...
            >
              ...
              >
                <h4 style={{ color: "#0369a1", fontWeight: 800, margin: 0 }}>
                  Video Content (iFrame Links)
                </h4>
                <button
                  onClick={() =>
                    setCurrentItem((prev) => ({
                      ...prev,
                      videoIframes: [...(prev.videoIframes || []), ""],
                    }))
                  }
                  style={{
                    background: "#0369a1",
              ...
              >
                {(currentItem.videoIframes || []).map((link, idx) => (
                  <div key={idx} style={{ display: "flex", gap: "0.5rem" }}>
                    ...
                    <button
                      onClick={() => {
                        const newLinks = currentItem.videoIframes.filter(
                          (_, i) => i !== idx,
                        );
                        setCurrentItem({
                          ...currentItem,
                ...
                {(!currentItem.videoIframes ||
                  currentItem.videoIframes.length === 0) && (
                  <p
                    style={{
                      color: "#64748b",
                      fontSize: "0.9rem",
            ...
            {/* Row 2: Text Content */}
            <div
              className={localStyles.formGrid}
              style={{ marginBottom: "2.5rem" }}
            >
              {/* English */}
              <div className={localStyles.formSection}>
                <h4 style={{ marginBottom: "1rem", color: "#DC143C" }}>
                  English Details
                </h4>
            ...
            <div style={{ paddingTop: "2rem", borderTop: "2px solid #f1f5f9" }}>
              <div
                className={localStyles.sliderManagement}
                style={{ marginTop: 0, paddingTop: 0, border: "none" }}
              >
                <label className={localStyles.label}>
                  Gallery / Slider (Photos & Uploaded Videos)
                </label>
                <div
                  className={localStyles.sliderGrid}
                ...
                >
                  {currentItem.sliderImages?.map((img, idx) => {
                    ...
                    return (
                      <div key={idx} className={localStyles.sliderItem}>
                        {videoMode ? (
                          <div
                            className={localStyles.sliderImg}
                            style={{
                              display: "flex",
                              flexDirection: "column",
                              alignItems: "center",
                        ...
                        <button
                          className={localStyles.removeSliderImg}
                          onClick={() =>
                            removeSliderImage(
                              idx,
                              idx < (currentItem.rawSliderImages?.length || 0),
                            )
                          }
                        >
                          <X size={14} />
                  ...
                  {/* Photo Add Button */}
                  <div
                    className={localStyles.addSliderBtn}
                    onClick={() => sliderImagesInputRef.current?.click()}
                    title="Upload Photo"
                  >
                    <Plus size={20} />
                  ...
                  <div
                    className={localStyles.addSliderBtn}
                    onClick={() =>
                      document.getElementById("directVideoInput").click()
                    }
                    title="Upload Video"
